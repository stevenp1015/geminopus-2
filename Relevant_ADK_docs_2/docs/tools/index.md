...
* `auth_response: Any`: Contains the credentials (e.g., a token) if authentication was already handled by the framework before your tool was called (common with RestApiTool and OpenAPI security schemes).
...
=== "Java"
    ...
    public Map<String, String> updateUserThemePreference(String value, ToolContext toolContext) {
      ...
      tool_context.state().put("user:preferences", preference);
